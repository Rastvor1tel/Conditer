...
$buldSubSection = function ($arSections) {
	$result = "";
	array_walk($arSections, function ($item) use (&$result) {
...
function buildSection($arSections) {
	$result = "";
	array_walk($arSections, function ($item) use (&$result) {
		$class = $item["CHILDS"] ? 'nav-item-submenu' : '';
		$checked = ($item["CHECKED"] || ($item['CHILD_SELECTED'] == 'Y')) ? 'checked="checked"' : '';
		//$disabled = $item["DISABLED"] ? 'disabled' : '';
		$subsection = "";
		if ($item["CHILDS"]) {
			$subsection .= "<ul class=\"nav nav-group-sub\">";
			$subsection .= buildSection($item["CHILDS"]);
			$subsection .= "</ul>";
		}
		$result .= <<<SECTION
			<li class="nav-item catalog_section {$class}">
				<a class="nav-link">
					<div class="form-check">
...
$buildSection = fn($arSections) => buildSection($arSections);
...
<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
	...
	<div class="card index_blank-categories">
		...
		<?php
		if ($arResult['ITEMS']['SECTION_ID']['SORT_VALUES']) {
			echo <<<SECTIONS
				<div class="card-body border-0 p-0">
					<ul class="nav nav-sidebar mb-2">
						{$buildSection($arResult['ITEMS']['SECTION_ID']['SORT_VALUES'])}
					</ul>
				</div>
			SECTIONS;
		}
									...
									foreach ($arItem["VALUES"] as $val => $ar):
										...
										<div class="bx_filter_parameters_box_checkbox form-check <? echo $ar["DISABLED"] ? 'disabled' : '' ?>">
											<label class="form-check-label checkbox__label fonts__middle_comment"
												...
												<div class="bx_filter_parameters_box_title fonts__middle_text font-size-xs text-uppercase text-muted">
                                                    <span>
                                                        <span class="item_name"><?= $arItem["NAME"] ?></span>
                                                    </span>
												</div>
					...
					<div class="bx_filter_block button">
						<div class="bx_filter_parameters_box_container filter_buttons">
							<div class="bx_filter_popup_result <?= $arParams["POPUP_POSITION"] ?>"
								...
								<? echo GetMessage("CT_BCSF_FILTER_COUNT", ["#ELEMENT_COUNT#" => '<span id="modef_num">' . intval($arResult["ELEMENT_COUNT"]) . '</span>']); ?>
							</div>
