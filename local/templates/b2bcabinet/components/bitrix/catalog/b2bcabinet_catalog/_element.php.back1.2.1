...
$rs = \CIBlockSection::GetList(
    ...
    true,
    ['ID','UF_DETAIL_TEMPLATE']
);
if ($section = $rs->GetNext()) {
    if ($section['UF_DETAIL_TEMPLATE'] > 0) {
        $rsEnum = \CUserFieldEnum::GetList(
            [],
            [
                'ID' => $section['UF_DETAIL_TEMPLATE'],
            ]
        );
        if ($enum = $rsEnum->GetNext()) {
            $template = $enum['XML_ID'];
        }
    }
}
...
if (isset($arParams['USE_COMMON_SETTINGS_BASKET_POPUP'])
    && $arParams['USE_COMMON_SETTINGS_BASKET_POPUP'] == 'Y'
) {
    ...
    $basketAction = (isset($arParams['DETAIL_ADD_TO_BASKET_ACTION'])
        ? $arParams['DETAIL_ADD_TO_BASKET_ACTION'] : []);
}
...
if(in_array("ADMIN", $arParams['ACTION_PRODUCTS']))
    $arParams['ACTION_PRODUCTS'] = Config::getArray("ACTION_PRODUCTS");

$componentElementParams = [
    'IBLOCK_TYPE'                => $arParams['IBLOCK_TYPE'],
    'IBLOCK_ID'                  => $arParams['IBLOCK_ID'],
    'OFFER_ID'                      => $arResult['VARIABLES']['OFFER_ID'],
    'OFFER_CODE'                    => $arResult['VARIABLES']['OFFER_CODE'],
