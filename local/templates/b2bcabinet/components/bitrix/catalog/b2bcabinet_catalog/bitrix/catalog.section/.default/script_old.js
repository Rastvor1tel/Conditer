...
$(document).ready(function () {
    ...
    for (let i = 0; i < touchspin_up.length; i++) {
        if ('ontouchstart' in window) {
            touchspin_up[i].addEventListener("touchstart", spinCount);
            touchspin_down[i].addEventListener("touchstart", spinCount);
        } else {
            touchspin_up[i].addEventListener("click", spinCount);
            touchspin_down[i].addEventListener("click", spinCount);
        }
    }

    function spinCount() {
        var dataObj = $(this).offsetParent().find('.form-control.touchspin-empty');

        if (dataObj.length > 0) {
            var productId = $(dataObj).data('id');
            var productQnt = $(dataObj).val();

            var mainBlock = $(this).parentsUntil('tbody').filter('tr');
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (items !== undefined && items !== '') {
                        var totalCount = 0;
                        if (items['TOTAL_COUNT'] !== undefined && items['TOTAL_COUNT'] !== '') {
                            totalCount = items['TOTAL_COUNT'];
                        }
                        $('.index_blank-add_cart-number').html(totalCount);

                        var totalPrice = 0;
                        if (items['TOTAL_PRICE'] !== undefined && items['TOTAL_PRICE'] !== '') {
    ...
    $.each(spin, function (key, val) {
        val.addEventListener(
            ...
            function () {
                ...
                this.timer = setTimeout(spinCount(), 700);
            });
    });
});
