	...
	<div class="table_header">
		...
		<div>
			<div class="blank_detail-search row index_blank-sorting">
                <?
                $APPLICATION->IncludeFile(SITE_DIR . "include/b2b_page_sort_catalog.php", [
                    $arResult,
                    $arParams,
                    "top"
                ], [
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href="<?= $methodIstall ?>orders/make/index.php">
						<button type="button" class="btn btn_b2b"><?= Loc::getMessage('POPUP_WAS_ADDED_PROCESSED') ?></button>
					</a>
				</div>
			</div>
		</div>
	...
	<script>
        ...
        if (!empty($GLOBALS[$arParams['FILTER_NAME']]['SECTION_ID']) && is_array($GLOBALS[$arParams['FILTER_NAME']]['SECTION_ID'])) {
            ...
            if (!empty($GLOBALS[$arParams['FILTER_NAME']]) && is_array($GLOBALS[$arParams['FILTER_NAME']])) {
                ...
                if (!empty($GLOBALS["sotbitFilterResult"]['ITEMS']['SECTION_ID']['VALUES']) && is_array($GLOBALS["sotbitFilterResult"]['ITEMS']['SECTION_ID']['VALUES'])) {
                    $filterProps = array_keys($GLOBALS["sotbitFilterResult"]['ITEMS']['SECTION_ID']['VALUES']);
                }
            }
        }
...
include_once($_SERVER["DOCUMENT_ROOT"] . $this->GetFolder() . "/modal.php");
?>