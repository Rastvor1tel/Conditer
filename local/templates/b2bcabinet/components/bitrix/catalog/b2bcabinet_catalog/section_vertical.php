...
$arAvailableSort = [
	...
	"name_0"    => ["name", "desc"],
	"name_1"    => ["name", "asc"],
	"price_0"   => ["property_minimum_price", "desc"],
	"price_1"   => ["property_minimum_price", "asc"],
	"date_0"    => ["date_create", "desc"],
	"date_1"    => ['date_create', "asc"],
	"articul_0" => ["articul", "desc"],
	"articul_1" => ["articul", "asc"],
];

if ($_POST['only_available'] == 'Y') {
	$_SESSION['only_available'] = 'Y';
} elseif ($_POST['only_available'] == 'N') {
	$_SESSION['only_available'] = 'N';
}

if ($_POST['only_checked'] == 'Y') {
	$_SESSION['only_checked'] = 'Y';
} elseif ($_POST['only_checked'] == 'N') {
...
$arSections = $arElements = [];

if (empty($arProducts['ID']) || !is_array($arProducts['ID'])) {
	$arElementsFilter = ["IBLOCK_ID" => $arParams['IBLOCK_ID'], "ACTIVE" => "Y"];
	$arElementsSelect = ["ID", "IBLOCK_SECTION_ID"];
...
if ($_SESSION['PRICE_ID']) {
	$resSection = CIBlockSection::GetList(false, ['IBLOCK_ID' => $arParams['IBLOCK_ID'], 'XML_ID' => $_SESSION['PRICE_ID']])->Fetch();
	if ($resSection) {
		$arSectionsFilter = [
...
if ($arSectionsFilter) {
	...
	$arSectionsSelect = ["ID", "IBLOCK_SECTION_ID", "DEPTH_LEVEL"];
	...
	if ($cache->initCache(36000000, $cacheId, '/sotbit.b2bcabinet')) {
		...
		while ($arRes = $res->Fetch()) {
			...
			$tmpSections['IBLOCK_SECTION_ID'] = ($tmpSections['DEPTH_LEVEL'] === '2') ? false : $arRes["IBLOCK_SECTION_ID"];
...
if ($isFilter || $isSidebar): ?>
	...
	<div class="sidebar sidebar-light bg-transparent sidebar-component sidebar-component-right border-0 shadow-0 sidebar-expand-md smartfilter_wrapper">
		<div class="sidebar-content bx_filter<?= (isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' hidden-xs' : '') ?>">
			<? if ($isFilter): ?>
				<?
				$APPLICATION->IncludeComponent("sotbit:b2bcabinet.catalog.smart.filter", "b2b_smart_filter", [
	...
	<div class="card">
		...
		<div class="card-body">
			<div class="card-excel-button">
				<?
				$APPLICATION->IncludeComponent("bitrix:main.file.input", "b2bcabinet_file_input", [
		...
		<div class="card-body price-select__block">
			<div>
				<form method="POST" action="">
					<span class="index_blank-sorting_title">Организация:</span>
					<select class="form-control select index_blank-sorting-select select2-hidden-accessible" name="ORGANIZATION_ID" onchange="this.form.submit()" tabindex="-1" aria-hidden="true">
						<option value="empty">Выберите организацию</option>
						<?
						foreach ($orgArray as $orgItem) {
							if ($_SESSION['ORGANIZATION_ID'] && ($orgItem['ID'] == $_SESSION['ORGANIZATION_ID'])) {
								echo "<option selected value=\"{$orgItem['ID']}\">{$orgItem['NAME']}</option>";
							} else {
								echo "<option value=\"{$orgItem['ID']}\">{$orgItem['NAME']}</option>";
							}
						}
						?>
					</select>
