...
function addControl(item) {
    ...
    newInput.className = "form-input-styled";
    newInput.onchange = function () {
        addControl(this);
    };
    var br = document.createElement("br");
    $(newInput).insertAfter(BX(item.id).parentNode);
    $(br).insertAfter(BX(item.id).parentNode);
    $(newInput).uniform();
}

BX.ready(function(){
    var organizationID = BX.getCookie('ORGANIZATION_ID');
    $("#PROFILE_ID_"+organizationID).click();
});
