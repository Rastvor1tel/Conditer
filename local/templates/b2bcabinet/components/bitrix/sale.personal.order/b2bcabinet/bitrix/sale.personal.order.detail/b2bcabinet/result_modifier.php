...
$docIblockId = \Bitrix\Main\Config\Option::get("sotbit.b2bcabinet", "DOCUMENT_IBLOCK_ID", "");
...
if (!empty($docIblockId) && !empty($arResult['ID'])) {
	...
	$arRes = CIBlockElement::GetList([], ['IBLOCK_ID' => $docIblockId, 'PROPERTY_ORDER' => $arResult['ID']]);
...
if (!empty($arResult['PAYMENT'])) {
	$filterOption = new Bitrix\Main\UI\Filter\Options('PAY_SYSTEMS_LIST');
	$filterData = [];
	$filterData = $filterOption->getFilter([]);
...
if (Loader::includeModule('support')) {
	$tickets = CTicket::GetList(
		$by = "ID",
		$order = "asc",
		['UF_ORDER' => $arResult['ID'], 'CREATED_BY' => $USER->GetID()],
		$isFiltered,
		"Y",
		"Y",
		"Y",
