...
require_once($_SERVER["DOCUMENT_ROOT"] . "/local/components/dial/sale.export.1c/functions.php");
...
$gzCompressionSupported = (($_GET["mode"] == "query" || $_POST["mode"] == "query") && $bCrmMode && isset($arParams["GZ_COMPRESSION_SUPPORTED"]) && $arParams["GZ_COMPRESSION_SUPPORTED"] && function_exists("gzcompress"));
...
if ($_GET["mode"] == "checkauth" && $USER->IsAuthorized()) {
    ...
    if ($_GET["mode"] == "init") {
        ...
        if ($_SESSION["BX_CML2_EXPORT"]["cmlVersion"] >= doubleval(\Bitrix\Sale\Exchange\ExportOneCBase::SHEM_VERSION_2_10)) {
            ...
            $arResultStat = $export::ExportOrders2Xml($arFilter, $nTopCount, $arParams["REPLACE_CURRENCY"], $bCrmMode, $arParams["INTERVAL"], $_SESSION["BX_CML2_EXPORT"]["version"], $options);
        }
    ...
    } elseif ($_GET["mode"] == "file" && strlen($_SESSION["BX_CML2_EXPORT"]["version"]) <= 0)// old version
    {
            ...
            if (isset($DATA) && $DATA !== false) {
                ...
                if ($fp = fopen($ABS_FILE_NAME, "ab")) {
                    ...
                    if ($result === (function_exists("mb_strlen") ? mb_strlen($DATA, 'latin1') : strlen($DATA))) {
                        ...
                        echo "failure\n" . GetMessage("CC_BSC1_ERROR_FILE_WRITE", ["#FILE_NAME#" => $FILE_NAME]);
                    }
                    fclose($fp);
                } else {
                    echo "failure\n" . GetMessage("CC_BSC1_ERROR_FILE_OPEN", ["#FILE_NAME#" => $FILE_NAME]);
                }
            } else {
        ...
        if (filesize($new_file_name) > 0) {
            ...
            $o->registerElementHandler("/" . GetMessage("CC_BSC1_COM_INFO"), [$loader, "elementHandler"]);
	...
	<<?= "?" ?>xml version="1.0" encoding="UTF-8"<?= "?" ?>>
        ...
        <?
        if (\Bitrix\Sale\Exchange\ManagerExport::isCRMCompatibility()) {
            $dbStatus = \Bitrix\Crm\Invoice\InvoiceStatus::getList(['order' => ["SORT" => "ASC"]]);
            while ($arStatus = $dbStatus->Fetch()) {
                ?>
            ...
            $dbStatus = CSaleStatus::GetList(["SORT" => "ASC"], ["LID" => LANGUAGE_ID], false, false, ["ID", "NAME"]);
            while ($arStatus = $dbStatus->Fetch()) {
                ?>
				<<?= GetMessage("CC_BSC1_DI_ELEMENT") ?>>
        ...
        <?
        $dbPS = CSalePaySystem::GetList(["SORT" => "ASC"], ["ACTIVE" => "Y"], false, false, [
            'ID',
            'NAME',
            'ACTIVE',
            'SORT',
...
if (!$bDesignMode) {
    ...
    if ($gzCompressionSupported) {
        ...
        if (in_array($_GET["mode"], ["query", "info"]) || in_array($_POST["mode"], ["query", "info"])) {
    ...
    $arStatuses = ["" => GetMessage("CP_BCI1_NO")];
