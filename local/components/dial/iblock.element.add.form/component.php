...
if (!is_array($arParams["PROPERTY_CODES"])) {
    ...
    foreach ($arParams["PROPERTY_CODES"] as $i => $k)
        if (strlen($k) <= 0) unset($arParams["PROPERTY_CODES"][$i]);
}
...
foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $value)
    if (strlen(trim($value)) <= 0) unset($arParams["PROPERTY_CODES_REQUIRED"][$key]);
...
if ($bAllowAccess) {
    ...
    if (check_bitrix_sessid() && (!empty($_REQUEST[$arParams['SUBMIT_NAME']]) || !empty($_REQUEST["iblock_apply"]))) {
        ...
        /*------spam_filter------*/
        if ($_REQUEST['secret-field'] != '') {
            ShowError('Подозрение на спам');
            $arResult["ERRORS"][] = 'Подозрение на спам';
        }
        /*------end of spam_filter------*/
        ...
        if (!$arResult["ERRORS"]) {
            ...
            if ($arParams["ID"] > 0) {
                ...
                //Bitrix\Main\Diag\Debug::writeToFile($arUpdateValues, '', '/local/php_interface/log.txt');
                $arReviewFields = Array(
                    "NAME" => $arUpdateValues['NAME'],
                    "EMAIL" => $arUpdateValues['PROPERTY_VALUES'][16],
                    "PHONE" => $arUpdateValues['PROPERTY_VALUES'][13],
                    "COMPANY" => $arUpdateValues['PROPERTY_VALUES'][18],
                    "REVIEW_TEXT" => $arUpdateValues['PREVIEW_TEXT']
                );
                CEvent::Send('REVIEW_NEW', "s1", $arReviewFields);
            }
        }
