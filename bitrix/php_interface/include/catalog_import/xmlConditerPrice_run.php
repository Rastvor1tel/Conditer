...
class xmlCatalogPriceImport {
    ...
    public function prepareElement($rsProduct, $IBLOCK_ID, $iblockID) {
        ...
        while ($arProduct = $rsProduct->Fetch()) {
            ...
            if (is_array($elementTemp['FIELDS'])) {
                if ($elementIDSKU) {
                    ...
                    $res = $bs->Add($elementTemp['FIELDS']);
                    $this->elementQuantity($res, $elementTemp['PRICE']);
                    $this->elementPrice($elementIDSKU, $formatPrice);
                }
            }
            unset($formatPrice, $elementIDSKU, $elementTemp, $arrElementTemp, $itemProps);
        }
    }
    ...
    if ($rsProductPrice->SelectedRowsCount() > 0) {
        while ($arProductPrice = $rsProductPrice->Fetch()) {
            ...
            if ($rsProduct->SelectedRowsCount() > 0) {
                ...
                $import->prepareElement($rsProduct, $IBLOCK_ID, $iblockID);
            }
