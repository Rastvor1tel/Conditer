...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() . $actionParams; ?>" ENCTYPE="multipart/form-data" name="dataload">
    ...
    if ($STEP == 1) {
		...
		<tr>
			...
			<td valign="top" width="60%"><?
                if (!isset($IBLOCK_ID))
                    $IBLOCK_ID = 0;
                echo GetIBlockDropDownListEx($IBLOCK_ID, 'IBLOCK_TYPE_ID', 'IBLOCK_ID', [
                    'CHECK_PERMISSIONS' => 'Y',
                    'MIN_PERMISSION'    => 'W'
                ], "", "", 'class="adm-detail-iblock-types"', 'class="adm-detail-iblock-list"');
                ?></td>
		</tr>
		<tr class="heading">
    ...
    if ($STEP < 2) {
		...
		<input type="hidden" name="SETUP_FIELDS_LIST" value="URL_FILE_1C,IBLOCK_ID,outFileAction">
